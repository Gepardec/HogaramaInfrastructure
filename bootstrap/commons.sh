...
main () {
  ...
  # getopts
  local opts=`getopt -o h --long oc-admin-token:,oc-cluster:,namespace:,help,dryrun -- "$@"`
  local opts_return=$?
  ...
  create_oc_resource "admin" ${namespace} "imagestream/jboss-eap/jboss-eap-7-is.json"
  create_oc_resource "admin" ${namespace} "imagestream/openjdk/openjdk-8-is.json"
  create_oc_resource "admin" ${namespace} "imagestream/redhat-sso/redhat-sso-7-is.json"
}
